...
import html2canvas from "html2canvas"; // <-- Adicionado html2canvas
import "../style.css";
...
function Calendar({
  ...
  const printRef = useRef(null); // <-- Referência para a área do print
  ...
  const handleDownloadImage = async () => {
    ...
    const canvas = await html2canvas(element, {
      backgroundColor: "#121212", // Cor de fundo da imagem gerada (ajuste se seu app usar outra)
  ...
  const handleDeleteMatch = (id) => {
    if (
      window.confirm(
        "Deseja realmente eliminar esta partida? Os pontos da tabela serão recalculados.",
      )
    ) {
  ...
  const hasMatchOnDate = (day) => {
    ...
    return matches.some((m) => m.date === fullDate);
  };
      ...
      <div className="glass-card">
        ...
        <div className="calendar-wrapper">
            ...
            <div className="calendar-grid">
              {getDaysInMonth().map((day, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`calendar-day ${day ? "" : "empty"} ${isToday ? "selected" : ""}`}
          ...
          <div className="matches-list-container">
            {/* Header Flex com Título e Botão Lado a Lado */}
            ...
            {/* ÁREA QUE SERÁ CAPTURADA PELO PRINT */}
                ...
                <div className="matches-feed">
                  {filteredMatches.map((match) => (
                    ...
                    >
                      {isAdmin && (
                        <div
                          data-html2canvas-ignore="true" // <-- Ignora os botões de Admin no Print!
                          style={{
                            display: "flex",
                        ...
                        >
                          <button
                            onClick={(e) => {
                              e.stopPropagation(); // Evita abrir a MatchPage ao clicar em editar
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation(); // Evita abrir a MatchPage ao clicar em eliminar
                            ...
                            .map((player) => {
                              ...
                              return (
                                <div
                                  ...
                                  style={{
                                    opacity:
                                      playerEvents.length > 0 || isGK ? 1 : 0.6,
                                  }}
                                >
                                  <span
